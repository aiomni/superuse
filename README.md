...
- **区域**：拖动框选；空格切换窗口模式；Esc 取消。一次区域选择在一块显示器内，支持多显示器和 Retina 原始像素。
- **窗口**：悬停自动高亮最前面的窗口，点击捕获。窗口截图不含阴影。
- **滚动**：框选可滚动的内容区，避开固定页眉和侧栏。手动缓慢向下滚动，每次保留至少 1/4 重叠，停稳后自动拼接。控制面板可暂停、继续、取消或完成；面板可以拖动且不会出现在截图中。
- **标注**：画笔、箭头、矩形、椭圆、文字、不透明黑色遮挡，支持颜色、粗细和撤销 / 重做。长图可滚动，双指缩放。
- **输出**：默认捕获后复制原图；标注后按 **⇧⌘C** 复制最终图片，**⌘S** 保存 PNG。导出保留原始像素尺寸。
...
自动测试覆盖：历史去重 / 容量 / 编辑 / 序列化、真实的隔离 NSPasteboard 读写、敏感标记、持久化删除、标注导出方向和撤销、多屏坐标换算、逐像素滚动拼接、静止和运动帧、歧义与容量上限。测试使用专用剪贴板和临时目录，不读写你的系统剪贴板历史。
